...
namespace Memories.Tests.Builders
{
    ...
    public class MovieAudioMergeArgsBuilderTest
    {
        ...
        public void 動画音声合成コマンドの引数が組み立てられる()
        {
            ...
            Assert.AreEqual("-i \"movie.MOV\" -stream_loop -1 -i \"audio.mp3\" -filter_complex \"[0:a]volume=4.0[a1];[1:a]volume=0.4[a2];[a1][a2]amix=inputs=2:duration=shortest\" -c:v copy -c:a aac -b:a 192k \"temp.MOV\"", actual);
        }
    }
}
